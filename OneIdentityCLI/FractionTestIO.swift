...
//===================================================
print("Fraction Calculator (Type 'exit' to quit)")

while true {
    print("Enter an operation (e.g., '3&1/4 + 1/2' or 'exit'): ", terminator: "")
    if let input = readLine(), input.lowercased() != "exit" {
       // let elements = input.split(separator: " ")
        print("Evaluating.... \(input)")
       
        do {
            let fractionResult = try IOFraction().processFraction(input: input)
            if fractionResult != "0" {
                print("Result: \(fractionResult)")
            } else {
                print("Error: Please add input in this format (1/2 * 3&3/4)")
            }
        } catch (let error) {
            if let errorRef = error as? IOFractionError {
                print("Error: \(errorRef.errorMessage)")
            } else {
